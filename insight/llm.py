...
def _build_agent_system(schema_info: list[dict], file_catalog: list[dict],
    ...
    if sql_mode:
        lines.append(
            ...
            "- For questions about what data is available (tables, columns, schemas): "
            "call run_sql with `SHOW TABLES IN <schema>` or `DESCRIBE <schema>.\"<table>\"` "
            "so the user can see the introspection query. Don't paraphrase the schema from memory.\n"
