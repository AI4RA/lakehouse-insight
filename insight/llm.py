...
_SQL_TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "every data lookup AND for schema introspection: simple SELECTs, joins, "
                "GROUP BY aggregates, window functions, CTEs, plus SHOW SCHEMAS / SHOW "
                "TABLES / DESCRIBE. Table references must be schema-qualified (see the "
                "system prompt for the exact schema). For introspection use the SHOW/"
                "DESCRIBE commands -- `information_schema` is BLOCKED by Marina (HTTP 403) "
                "and a bare `SHOW SCHEMAS` is also rejected; use the LIKE-filtered form. "
                "Returns column names, a sample of rows, and total row count."
            ),
            "parameters": {
                "type": "object",
...
def _build_agent_system(schema_info: list[dict], file_catalog: list[dict],
    ...
    if sql_mode:
        lines.append(
            ...
            "- Introspection (tables / columns): call run_sql with SHOW/DESCRIBE so the "
            "user sees the query. Don't paraphrase the schema from memory.\n"
            ...
            "- SHOW SCHEMAS returns all-tenant catalog metadata -- it does NOT reflect your "
            "authorized access. Schemas visible in SHOW SCHEMAS output (e.g. bronze, silver, "
            "gold, system) are NOT queryable with your bearer token. Never present SHOW SCHEMAS "
            "results as 'what you can see'. Only tables you can successfully SELECT from in "
            f"`{schema_qual}` are part of your data access.\n"
            "- `information_schema`, `pg_catalog`, and bare `SHOW SCHEMAS` (without a LIKE "
            "clause) are BLOCKED by Marina. Do not attempt them.\n"
